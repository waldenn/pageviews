  ...
  _createClass(UserViews, [{
    ...
    value: function setupNamespaceSelector() {
      ...
      this.fetchSiteInfo(this.project).then(function (data) {
        $('#namespace-select').html('<option value=\'all\'>' + $.i18n('all') + '</option>');

        var namespaces = data[_this3.project].namespaces;
        for (var ns in namespaces) {
          if (ns < 0) continue;
          var nsTitle = namespaces[ns]['*'] || $.i18n('main');
          $('#namespace-select').append('<option value=' + ns + '>' + nsTitle + '</option>');
        }
    ...
    value: function pushParams() {
      ...
      _get(UserViews.prototype.__proto__ || Object.getPrototypeOf(UserViews.prototype), 'pushParams', this).call(this, 'user', clear);
    }
    ...
    value: function getPageViewsData(pages) {
      ...
      var makeRequest = function makeRequest(pageObj) {
        ...
        var url = 'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/' + _this9.project + ('/' + _this9.$platformSelector.val() + '/' + _this9.$agentSelector.val() + '/' + uriEncodedPageName + '/daily') + ('/' + startDate.format(_this9.config.timestampFormat) + '/' + endDate.format(_this9.config.timestampFormat));
        ...
        promise.done(function (pvData) {
          ...
          if (cassandraError) {
            ...
            _this9.writeMessage(failedPageLink + ': ' + $.i18n('api-error', 'Pageviews API') + ' - ' + errorMessage);
          }

          // unless it was a 404, don't cache this series of requests
          if (errorData.status !== 404) _this9.hadFailure = true;
        }).always(function () {
    ...
    value: function processInput() {
      ...
      if (this.isRequestCached()) {
        ...
        return setTimeout(function () {
          _this11.outputData = simpleStorage.get(_this11.getCacheKey());
          readyForRendering();
        }, 500);
      }
      ...
      this.getPagesCreated(user).done(function (pagesCreated) {
        if (!pagesCreated.length) {
          _this11.toastInfo($.i18n('select2-no-results'));
          return _this11.setState('initial');
        }
        _this11.getPageViewsData(pagesCreated).done(function (pageViewsData) {
          $('.progress-bar').css('width', '100%');
          $('.progress-counter').text($.i18n('building-dataset'));
          var userLink = _this11.getUserLink(user);
          setTimeout(function () {
            _this11.buildMotherDataset(user, userLink, pageViewsData);
            readyForRendering();
          }, 250);
        });
      }).fail(function (error) {
        ...
        if (typeof error === 'string') {
          ...
          _this11.toastError($.i18n('api-error-unknown', 'Page Creation API'));
        }
      }).always(function () {
        $('.elapsed-timer').text('');
        window.clearInterval(_this11.elapsedTimer);
        _this11.elapsedTimer = null;
      });
    }
